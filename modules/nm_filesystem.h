 ...
 * @brief Filesystem traversal, safe temporary files, backups, and atomic installation.
 *
...
/**
 * @brief Writes an exact byte sequence to a file descriptor, retrying interrupted writes.
...
/**
 * @brief Atomically at descriptor level creates/truncates a file, writes bytes, and enforces its mode.
 * @param path Filesystem or procfs path.
 * @param data Source byte sequence.
 * @param len Number of source bytes.
 * @param mode POSIX permission mode.
 * @return Zero or a positive success value as documented by the implementation; a negative/zero failure value otherwise.
 */
NM_INTERNAL int write_file_mode(const char *path, const void *data, size_t len, mode_t mode);

/**
 * @brief Appends an exact byte sequence to a file, creating it with private permissions if absent.
 * @param path Filesystem or procfs path.
 * @param data Source byte sequence.
 * @param len Number of source bytes.
 * @return Zero or a positive success value as documented by the implementation; a negative/zero failure value otherwise.
...
/**
 * @brief Copies a regular file while preserving permissions and timestamps where possible.
 ...
 * @return Zero or a positive success value as documented by the implementation; a negative/zero failure value otherwise.
 */
NM_INTERNAL int copy_regular_file(const char *src, const char *dst, const struct stat *st);

/**
 * @brief Recursively copies a directory tree while preserving metadata where possible.
 * @param src Source filesystem path.
 * @param dst Destination filesystem path or destination buffer.
 * @param st Metadata obtained for the source path.
 * @return Zero or a positive success value as documented by the implementation; a negative/zero failure value otherwise.
 */
NM_INTERNAL int copy_directory(const char *src, const char *dst, const struct stat *st);

/**
 * @brief Copies a supported regular file, directory, or symbolic link recursively.
 * @param src Source filesystem path.
 * @param dst Destination filesystem path or destination buffer.
 * @return Zero or a positive success value as documented by the implementation; a negative/zero failure value otherwise.
 */
...
/**
 * @brief Verifies that a configuration file carries the manager ownership marker.
 * @param path Filesystem or procfs path.
 * @return Nonzero when the predicate is true; otherwise zero.
 */
 ...
 */
NM_INTERNAL void record_change(char type, const char *target, const char *backup);
...
/**
 * @brief Applies overwrite policy and creates a rollback backup before replacement.
 * @param target Live target path or generated target identifier.
 */
NM_INTERNAL void prepare_for_replace(const char *target);

/**
 * @brief Validates permissions and atomically installs the active temporary file.
 * @param target Live target path or generated target identifier.
 * @param mode POSIX permission mode.
 */

...
/**
 * @brief Returns a newly allocated line with exact endpoint port tokens replaced.
 * @param line One Nginx configuration line.
 * @param old_port Existing TCP port.
 * @param new_port Replacement TCP port.
 * @return A newly allocated string that the caller must release with free(), or NULL on failure.
 */
NM_INTERNAL char * replace_port_tokens_alloc(const char *line, unsigned old_port, unsigned new_port) NM_MALLOC;
 ...
 * @return A newly allocated string that the caller must release with free(), or NULL on failure.
 */
NM_INTERNAL char * replace_literal_alloc(const char *line, const char *old, const char *replacement) NM_MALLOC;
 ...
 * @return A newly allocated string that the caller must release with free(), or NULL on failure.
 */
NM_INTERNAL char * replace_listen_endpoint_alloc(const char *line, unsigned old_port, unsigned new_port) NM_MALLOC;
 ...
 */
NM_INTERNAL intS replace_listener_port(const char *source, unsigned old_port, unsigned new_port, const char *output);

/**
 * @brief Builds a managed-file candidate with one selected upstream server port changed.
 * @param source Source configuration file path or source text.
 ...
 * @return Zero or a positive success value as documented by the implementation; a negative/zero failure value otherwise.
 */
NM_INTERNAL intS replace_upstream_port_candidate(const char *source, const char *wanted_name, const char *wanted_target, unsigned old_port, unsigned new_port, const char *output);
 ...
 * @return Zero or a positive success value as documented by the implementation; a negative/zero failure value otherwise.
 */
NM_INTERNAL intS remove_listener_candidate(const char *source, const char *wanted_listen, const char *wanted_server, const char *output);
...
/**
 * @brief Builds a candidate with a selected upstream and dependent routes removed.
 * @param source Source configuration file path or source text.
 * @param wanted_name Selected upstream name.
 * @param output Generated candidate output path.
 * @return Zero or a positive success value as documented by the implementation; a negative/zero failure value otherwise.
 */
NM_INTERNAL intS remove_upstream_candidate(const char *source, const char *wanted_name, const char *output);
...
/**
 * @brief Displays selectable managed nodes and performs one requested management action.
 * @return Zero or a positive success value as documented by the implementation; a negative/zero failure value otherwise.
 */

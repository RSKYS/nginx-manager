/**
 * @file nm_process.h
 * @brief Process execution, executable discovery, port ownership, and randomness.
 *
...
/**
 * @brief Resolves an executable name through PATH or validates an explicit path.
...
/**
 * @brief Forks and executes a command, optionally suppressing or capturing output.
 * @param argv NULL-terminated argument vector; element zero names the executable.
 * @param quiet Nonzero to redirect uncaptured child output to /dev/null.
 * @param capture Optional initialized buffer receiving combined stdout and stderr.
 * @return Zero or a positive success value as documented by the implementation; a negative/zero failure value otherwise.
 */
...
/**
 * @brief Scans a Linux procfs TCP table for a listening port and optionally records socket inodes.
 * @param path Filesystem or procfs path.
 * @param port TCP port number.
 * @param inodes Optional destination vector for matching socket inode strings.
 * @return Zero or a positive success value as documented by the implementation; a negative/zero failure value otherwise.
 */
NM_INTERNAL intS read_proc_listen_table(const char *path, unsigned port, StrVec *inodes);
...
/**
 * @brief Returns a best-effort 32-bit random value using the kernel RNG with a deterministic fallback.

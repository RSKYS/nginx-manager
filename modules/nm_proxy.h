/**
 * @file nm_proxy.h
 * @brief Reverse-proxy layout collection, upstream discovery, and site generation.
 *
...
/**
 * @brief Appends standard reverse-proxy headers, timeout handling, and forwarding metadata.
 ...
 */
NM_INTERNAL void append_common_proxy_headers(Buffer *b, unsigned forwarded_port, const char *timeout);
...
/**
 * @brief Tests whether a generated fragment already contains an exact location block.
 * @param b Buffer object.
...
/**
 * @brief Interactively collects static, proxy, WebSocket, and custom location blocks.
 * @param forwarded_port Client-facing port written into forwarding headers.
 */
NM_INTERNAL void collect_custom_locations(unsigned forwarded_port);

/**
 * @brief Selects a proxy layout and generates the corresponding location fragment.
 * @param forwarded_port Client-facing port written into forwarding headers.
 */
...
/**
 * @brief Recursively searches configuration paths for a named upstream definition.
 ...
 */
NM_INTERNAL intS find_upstream_recursive(const char *dir, const char *wanted, const char *excluded, char *found, size_t cap);
...
/**
 * @brief Collects referenced upstream definitions into a generated target fragment.
 * @param locations Generated location-fragment path.
 * @param target Live target path or generated target identifier.
 */
NM_INTERNAL void collect_required_upstreams(const char *locations, const char *target);
...
/**
 * @brief Generates, validates, and installs an HTTPS virtual-host configuration.
 * @param listen Nginx listen directive value.
 * @param target Live target path or generated target identifier.
 * @return Zero or a positive success value as documented by the implementation; a negative/zero failure value otherwise.
 */
NM_INTERNAL intS write_https_site_config(const char *listen, const char *target);

/**
 * @brief Generates, validates, and installs a custom-port HTTP/HTTPS configuration.
 * @param target Live target path or generated target identifier.
 * @return Zero or a positive success value as documented by the implementation; a negative/zero failure value otherwise.
 */
